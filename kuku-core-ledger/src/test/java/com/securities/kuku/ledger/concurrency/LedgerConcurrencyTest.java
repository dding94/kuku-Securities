...
/** Ledger 동시성 테스트. 동일 계좌에 대한 동시 입출금 시 데이터 정합성을 검증합니다. */
@SpringBootTest
@ActiveProfiles("test")
...
class LedgerConcurrencyTest {
  ...
  class ConcurrentMixedTest {
    ...
    void 동시_입출금_혼합시_정합성_유지() {
      ...
      ExecutionResult result =
          ConcurrencyRunner.run(
              depositThreads + withdrawThreads,
              () -> {
                ...
                if (index < depositThreads) {
                  DepositCommand command =
                      DepositCommand.of(accountId, txAmount, "Mixed deposit", businessRefId);
                  depositService.deposit(command);
                } else {
                  WithdrawCommand command =
                      WithdrawCommand.of(accountId, txAmount, "Mixed withdraw", businessRefId);
  ...
  class LostUpdatePreventionTest {
    ...
    void 연속_업데이트_모두_반영() {
      ...
      BigDecimal expectedBalance =
          initialBalance.subtract(withdrawAmount.multiply(new BigDecimal(successCount)));
      fixture.assertBalance(accountId, expectedBalance);
