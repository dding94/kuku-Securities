...
/**
 * Ledger 도메인 테스트를 위한 Fixture 및 Helper 클래스. 테스트 데이터 생성, 정리(CleanUp), 정합성 검증을 담당합니다.
 *
 * <p>개선사항: - testRunId를 통한 테스트 격리 - SUM(amount) 기반의 재무적 정합성 검증 - CleanUp 로깅
 *
 * <p>주의: @Component가 아닌 @Import로 빈을 등록합니다. (테스트 전용 클래스)
 */
public class LedgerTestFixture {
  ...
  public Long createAccountWithBalance(BigDecimal amount) {
    ...
    AccountJpaEntity account =
        new AccountJpaEntity(
            accountId, 100L, "ACC-" + accountId, "KRW", AccountType.USER_CASH, FIXED_TIME);
    entityManager.persist(account);

    BalanceJpaEntity balance =
        new BalanceJpaEntity(accountId, amount, BigDecimal.ZERO, null, null, FIXED_TIME);
  ...
  /** 해당 계좌와 관련된 모든 데이터를 정리합니다. EntityManager를 사용하여 직접 JPQL을 실행하므로 Repository 인터페이스를 오염시키지 않습니다. */
  @Transactional
  public void cleanup(Long accountId) {
    ...
    int deletedTransactions =
        entityManager
            .createQuery(
                "DELETE FROM TransactionJpaEntity t WHERE t.businessRefId LIKE :refPattern")
            .setParameter("refPattern", refPattern)
            .executeUpdate();

    int deletedBalances =
        entityManager
            .createQuery("DELETE FROM BalanceJpaEntity b WHERE b.accountId = :accountId")
            .setParameter("accountId", accountId)
            .executeUpdate();

    int deletedAccounts =
        entityManager
            .createQuery("DELETE FROM AccountJpaEntity a WHERE a.id = :accountId")
            .setParameter("accountId", accountId)
            .executeUpdate();

    log.info(
        "Cleanup for account {}: Deleted {} Journals, {} Tx, {} Balances, {} Accounts",
        accountId,
   ...
   */
  public void assertLedgerConsistency(
      Long accountId, int expectedSuccessCount, BigDecimal amountPerTx) {
    String refPattern = String.format("Tx-%s-%d-%%", testRunId, accountId);
    ...
    Long txCount =
        entityManager
            .createQuery(
                "SELECT COUNT(t) FROM TransactionJpaEntity t WHERE t.businessRefId LIKE :refPattern",
                Long.class)
            .setParameter("refPattern", refPattern)
            .getSingleResult();

    assertThat(txCount)
        .as("Transaction count should match success count")
    ...
    Long journalCount =
        entityManager
            .createQuery(
                "SELECT COUNT(e) FROM JournalEntryJpaEntity e WHERE e.accountId = :accountId",
                Long.class)
            .setParameter("accountId", accountId)
            .getSingleResult();

    assertThat(journalCount)
        .as("JournalEntry count should match success count")
        .isEqualTo(expectedSuccessCount);
    ...
    assertThat(actualTotalAmount)
        .as("Total Journal amount should match verification")
        .isEqualByComparingTo(expectedTotalAmount);
  }

  /**
   * 해당 계좌의 DEBIT 합계가 기대값과 일치하는지 검증합니다. 출금(Withdraw) 테스트에서 사용 - 출금은 DEBIT으로 기록됩니다.
   *
   * @param accountId 검증 대상 계좌 ID
  ...
  /**
   * 해당 계좌의 CREDIT 합계가 기대값과 일치하는지 검증합니다. 입금(Deposit) 테스트에서 사용 - 입금은 CREDIT으로 기록됩니다.
   *
   * @param accountId 검증 대상 계좌 ID
  ...
  /**
   * 해당 계좌의 DEBIT 합계를 반환합니다.
   *
   * @param accountId 조회 대상 계좌 ID
   * @return DEBIT 합계 (없으면 0)
   */
  public BigDecimal getDebitTotal(Long accountId) {
  ...
  /**
   * 해당 계좌의 CREDIT 합계를 반환합니다.
   *
   * @param accountId 조회 대상 계좌 ID
   * @return CREDIT 합계 (없으면 0)
   */
  public BigDecimal getCreditTotal(Long accountId) {
    return entityManager
        .createQuery(
            "SELECT COALESCE(SUM(e.amount), 0) FROM JournalEntryJpaEntity e "
                + "WHERE e.accountId = :accountId AND e.entryType = 'CREDIT'",
